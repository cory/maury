	...
	for (var v=0;v<voyages.length;v++) {
		...
		if (voyage[1] <= end && voyage[2] >= start) {
			for (var p=3;p<voyage.length-5;p += 5) {
				var dist_lat_lon = getDistanceFromLatLonInKm(voyage[p+3],voyage[p+4],voyage[p+5+3],voyage[p+5+4]);
				...
				var distance_screen = distance(drawx0,drawy0,drawx1,drawy1);
				if (distance_screen > 0 && distance_screen < 50 && voyage[p+5+2] > 0) {
				  dist_lat_lon /= (50*voyage[p+5+2]);
				  drawx1 = ((drawx1-drawx0)/distance_screen*dist_lat_lon+drawx0)|0;
				  drawy1 = ((drawy1-drawy0)/distance_screen*dist_lat_lon+drawy0)|0;
				  if (drawx0 != drawx1 || drawy0 != drawy1) {
						draw_line(drawx0,drawy0,drawx1,drawy1,dist_lat_lon);
					  found = true;
				  }
				}
			}
